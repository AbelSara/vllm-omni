...
Requirements:
    ...
    - The deploy YAML at vllm_omni/deploy/hunyuan_image_3_moe.yaml
      (prefetch is enabled inside stage-1's omni_kv_config in the deploy YAML)
"""
...
def _main() -> None:
    ...
    # Enable prefetch on the DiT receive stage.  The deploy YAML
    # (hunyuan_image_3_moe.yaml) contains commented-out prefetch keys inside
    # stage-1's omni_kv_config block.  To run with prefetch, uncomment those
    # lines in the YAML, or provide a separate deploy config with them set.
    if not args.no_prefetch:
        # TODO: Provide a prefetch-enabled deploy YAML, or add per-stage
        # omni_kv_config override support to the Omni constructor.
        pass
